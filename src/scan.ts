...
export async function scanRepo(options: ScanOptions): Promise<ScanResult> {
  ...
  const commands = rankCommands(detection.files, detection.scripts, detection.nodePackageManagers);
  ...
  return {
    ...
    commands,
    findings: findingsFor(commands, detection.nodePackageManagers)
  };
}
