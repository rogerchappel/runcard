# Changelog

All notable changes to this project will be documented in this file.

This project follows the [Keep a Changelog](https://keepachangelog.com/en/1.1.0/)
format and uses semantic versioning when versioned releases are published.

## [Unreleased]

### Added

- Added packed-artifact installation and CLI execution to the package smoke check.
- Added npm trusted publishing with provenance to tagged releases.
- Clarified installation instructions while the first npm release is pending.
- Preserved the `runcard` executable when npm publishes the package.
- Added CLI release tests for top-level help, fixture-backed JSON output, and unknown fixture failures.
- Added a release-readiness checklist for local verification and package review.

- Initial project setup.

### Fixed

- Render directly executable workspace script commands with npm, pnpm, yarn, and bun's manager-specific path or package-name selectors.
- Run compiled tests through deterministic Node-based discovery so the advertised Node 20 minimum is supported and verified in CI alongside Node 24.
- Made top-level `runcard --help` exit successfully for package smoke checks and CLI discovery.
- Emit detected Node scripts as package-manager invocations so local binaries and lifecycle hooks work, while retaining raw manifest bodies as `scriptBody` metadata in JSON schema version 2.

## Release Links

- Unreleased:
  `https://github.com/rogerchappel/runcard/compare/...HEAD`
- Latest release:
  `https://github.com/rogerchappel/runcard/releases/latest`

Replace placeholder links once the first release tag exists.
